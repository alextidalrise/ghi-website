...
// Query: {    "guide": *[      _type == "guide"      && country == $countrySlug      && guideCategory == "buying"      && defined(slug.current)    ] | order(coalesce(order, 999) asc, title asc)[0] {  _id,  title,  "slug": slug.current},    "partners": *[      _type == "partner"      && defined(slug.current)      && count(categories[@->slug.current in $partnerCategories]) > 0      && $countrySlug in countries    ] | order(coalesce(order, 999) asc, name asc) {  _id,  name,  "slug": slug.current,  "categories": categories[]->name,  "categorySlugs": categories[]->slug.current}  }
...
// Query: *[_type == "guide" && defined(slug.current)]    | order(coalesce(order, 999) asc, title asc) {  _id,  title,  "slug": slug.current,  guideCategory,  audienceLabel,  tagline,  heroImage{  asset,  fileAsset,  altText,  "lqip": asset.asset->metadata.lqip,  "dimensions": asset.asset->metadata.dimensions}}
...
// Query: *[    _type == "locationTaxonomy"    && type == "country"    && defined(slug.current)  ] | order(name asc){    name,    "slug": slug.current,    tagline,    "flagUrl": flag.asset->url  }
