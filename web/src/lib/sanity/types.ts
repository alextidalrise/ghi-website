...
// Query: {    "guide": *[      _type == "guide"      && country == $countrySlug      && guideCategory == "buying"      && defined(slug.current)    ] | order(coalesce(order, 999) asc, title asc)[0] {  _id,  title,  "slug": slug.current},    "partners": *[      _type == "partner"      && defined(slug.current)      && defined(logo.asset)      && category->slug.current in $partnerCategories    ] | order(coalesce(order, 999) asc, name asc) {  _id,  name,  "slug": slug.current,  "category": category->name,  "categorySlug": category->slug.current,  logo{  asset,  fileAsset,  altText,  "lqip": asset.asset->metadata.lqip,  "dimensions": asset.asset->metadata.dimensions}}  }
...
// Query: *[    _type == "golfCourse"    && slug.current == $slug    && community->slug.current == $communitySlug    && community->parent->slug.current == $locationSlug    && community->parent->parent->slug.current == $countrySlug    &&   (coalesce(reviewStatus, "draft") == "approved" || $previewAll)  ][0]{  _id,  name,  "slug": slug.current,  shortDescription,  tagline,  seoTitle,  metaDescription,  holes,  par,  designStyle,  websiteUrl,  media[]{  asset,  fileAsset,  altText,  "lqip": asset.asset->metadata.lqip,  "dimensions": asset.asset->metadata.dimensions},  community->{    _id,    name,    "slug": slug.current,    breadcrumbLabel,    "locationSlug": parent->slug.current,    "countrySlug": parent->parent->slug.current,    parent->{      _id,      name,      "slug": slug.current,      breadcrumbLabel    },    "country": parent->parent->{      _id,      name,      "slug": slug.current,      breadcrumbLabel    }  }}
...
// Query: *[    _type == "partner"    && defined(slug.current)    && defined(logo.asset)  ] | order(coalesce(order, 999) asc, name asc)[0...$limit] {  _id,  name,  "slug": slug.current,  "category": category->name,  "categorySlug": category->slug.current,  logo{  asset,  fileAsset,  altText,  "lqip": asset.asset->metadata.lqip,  "dimensions": asset.asset->metadata.dimensions}}
