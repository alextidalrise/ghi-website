...
// Query: {    "guide": *[      _type == "guide"      && country == $countrySlug      && guideCategory == "buying"      && defined(slug.current)    ] | order(coalesce(order, 999) asc, title asc)[0] {  _id,  title,  "slug": slug.current},    "partners": *[      _type == "partner"      && defined(slug.current)      && category->slug.current in $partnerCategories      && $countrySlug in countries    ] | order(coalesce(order, 999) asc, name asc) {  _id,  name,  "slug": slug.current,  "category": category->name,  "categorySlug": category->slug.current}  }
...
// Query: *[_type == "partner" && slug.current == $slug][0]{    name,    "slug": slug.current,    "category": category->name  }
