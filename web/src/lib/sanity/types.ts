...
// Query: *[    _type == "locationTaxonomy"    && type == "country"    && slug.current == $countrySlug  ][0]{    "locations": featuredLocations[        @->_type == "locationTaxonomy"  && @->type == "location"  && defined(@->slug.current)  && defined(@->parent->slug.current)    ]->{  _id,  name,  "slug": slug.current,  type,  breadcrumbLabel,  tagline,  heroImage{  asset,  fileAsset,  altText,  "lqip": asset.asset->metadata.lqip,  "dimensions": asset.asset->metadata.dimensions},  "countrySlug": parent->slug.current,  "countryName": parent->name}  }
