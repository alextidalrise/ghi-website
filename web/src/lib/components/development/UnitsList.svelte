<script lang="ts">
	...
	let { units, showPricing = true }: Props = $props();
...
{#if units.length > 0}
	<section class="inventory content-wrap" aria-labelledby="units-heading">
		<h2 id="units-heading">Available units</h2>
		<ul class="inventory__list">
			{#each units as unit (unit._id)}
				{@const name = unit.unitName ?? 'Unit'}
				{@const number = unit.unitNumber}
				{@const specs = unit.specs as Specs}
				{@const price = showPricing ? formatListingPrice(unit.pricing) : null}
				{@const availability = formatEnumLabel(unit.pricing?.availabilityStatus)}
				...
				<li class="inventory__item">
					...
					<div class="inventory__body">
						<h3>
							{name}{#if number}
								<span class="inventory__number"> · {number}</span>
							{/if}
						</h3>
