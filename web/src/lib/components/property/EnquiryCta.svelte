<script lang="ts">
	...
	const showBrochure = $derived(
		ctas?.brochureCtaEnabled && listing.media?.brochure && listing.media.brochureVisibility === 'public_approved'
	);
</script>

<section class="enquiry" aria-labelledby="enquiry-heading" id="enquire">
	<div class="content-wrap enquiry__inner">
		<div class="enquiry__copy">
			<p class="enquiry__overline text-overline">Enquire</p>
			<h2 id="enquiry-heading">{primaryLabel}</h2>
		...
		<div class="enquiry__actions">
			...
			{/if}
			{#if showBrochure && ctas?.brochureCtaText}
		...
		<form class="enquiry__form" id="enquire-form" action="#" method="post">
			<p class="enquiry__form-note">
				Enquiry form wiring is out of scope for v1. Reference: {listing.ghiListingId}
			</p>
			<label>
				<span>Name</span>
