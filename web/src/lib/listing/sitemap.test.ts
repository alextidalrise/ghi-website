...
describe('collectSitemapEntries excludes units', () => {
	/* Units are canonicalised to their parent development (buildUnitDetailPageData), so a unit
	   URL must never be submitted — a sitemap entry and a rel=canonical pointing elsewhere are
	   contradictory signals. collectSitemapEntries takes no unit rows at all now; this guards
	   against a development's own entry accidentally growing a unit segment. */
	it('emits the development path only, never a nested unit path', () => {
		const entries = collectSitemapEntries(
			[],
			[
				{
		...
		expect(paths).toContain('/portugal/vilamoura/natura-village');
